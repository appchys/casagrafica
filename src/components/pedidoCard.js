...
export function renderPedidoCard(pedido, options = {}) {
  ...
  if (pedido.isGrupo) {
    // Grupo de pedidos unificados
    const subPedidosHTML = pedido.pedidos.map((sub) => {
      const maxPreview = 3;
      const preview = sub.productos.slice(0, maxPreview)
        .map(p => `<div>${p.producto_tipo}${p.detalle_personalizado ? ` — ${p.detalle_personalizado}` : ''}</div>`)
        .join('');
      const extra = sub.productos.length > maxPreview
        ? `<div style="font-size:0.75rem; color:var(--text-tertiary);">+${sub.productos.length - maxPreview} más</div>`
        : '';

      const adjuntos = sub.adjuntos || [];
      const adjuntosHTML = adjuntos.length > 0
        ? `
          <div class="pedido-card-attachments" style="margin-top: 8px; display: flex; flex-wrap: wrap; gap: 6px; padding-top: 6px; border-top: 1px dashed var(--border);">
            ${adjuntos.map((adj) => {
              ...
              return `
                <div onclick="event.stopPropagation(); window.mostrarDetalleAdjunto('${adj.nombre.replace(/'/g, "\\'")}', '${adj.url}', '${adj.tipo || ''}', '${subidoPor.replace(/'/g, "\\'")}', '${adj.fecha || ''}')" class="pedido-attachment-item" title="Ver detalles de ${adj.nombre}" style="display: inline-flex; align-items: center; gap: 6px; padding: 2px 6px; background: var(--bg-body, #f5f5f5); border-radius: 4px; font-size: 0.7rem; color: var(--text-secondary, #666); cursor: pointer; max-width: 100%; border: 1px solid var(--border); transition: all 0.15s; overflow: hidden;" onmouseover="this.style.background='var(--border)'; this.style.borderColor='var(--border-strong)'; this.style.color='var(--text-primary)'" onmouseout="this.style.background='var(--bg-body, #f5f5f5)'; this.style.borderColor='var(--border)'; this.style.color='var(--text-secondary, #666)'">
                  ...
                  <a href="${adj.url}" target="_blank" download="${adj.nombre}" onclick="event.stopPropagation()" class="pedido-attachment-download" title="Descargar archivo" style="display: inline-flex; align-items: center; color: var(--text-tertiary); text-decoration: none; padding: 2px; border-radius: 2px; transition: all 0.15s;" onmouseover="this.style.color='var(--accent, #e53935)'; this.style.background='var(--border)'" onmouseout="this.style.color='var(--text-tertiary)'; this.style.background='transparent'">
      ...
      // Taller actions
      let tallerActionsHTML = '';
      if (showTallerActions && sub.estado_produccion !== 'ENTREGADO') {
        tallerActionsHTML = `
          <div style="flex-shrink:0; display: flex; gap: 4px;">
            ${sub.estado_produccion === 'PENDIENTE' ? `
              <button type="button" class="btn btn-icon-round btn-info taller-card-action" data-taller-action="comenzar" data-doc-id="${sub._docId}" title="Comenzar producción" style="width:28px; height:28px; padding:0; display:flex; align-items:center; justify-content:center;">
                <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="width:12px; height:12px;"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3zM7 22H4a2 2 0 0 1-2-2v-7a2 2 0 0 1 2-2h3"></path></svg>
              </button>
            ` : ''}
            ${(sub.estado_produccion === 'LISTO' || sub.estado_produccion === 'EN PROCESO') ? `
              <button type="button" class="btn btn-icon-round btn-success taller-card-action" data-taller-action="entregar" data-doc-id="${sub._docId}" title="Entregar" style="width:28px; height:28px; padding:0; display:flex; align-items:center; justify-content:center;">
                <svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="width:12px; height:12px;"><polyline points="20 6 9 17 4 12"></polyline></svg>
              </button>
            ` : ''}
          </div>
        `;
      ...
      const estadoPagoHTML = sub.saldo_pendiente > 0
        ? `<span class="pedido-card-amount pedido-card-amount-action" data-abono-pedido-id="${sub._docId}" style="color:var(--danger-text);" title="Registrar pago o abono">
            <svg class="icon icon-abono" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 12px; height: 12px; margin-right: 4px; display: inline-block; vertical-align: middle;">
              <rect x="2" y="6" width="20" height="12" rx="2"></rect>
              <circle cx="12" cy="12" r="2"></circle>
              <path d="M6 12h.01M18 12h.01"></path>
            </svg>
            <span style="vertical-align: middle;">${formatCurrency(sub.saldo_pendiente)}</span>
           </span>`
        : `<span class="pedido-card-amount" style="color:var(--success-text); font-weight: 700;">✓ Pagado</span>`;

      return `
        <div class="sub-pedido-block" style="padding: 12px 0; border-top: 1px solid var(--border); margin-top: 8px;" data-doc-id="${sub._docId}">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 8px;">
            <div style="display:flex; align-items:center; gap:6px;">
              <span class="pedido-card-id" style="font-size:0.82rem; font-weight:700; background: var(--bg-body); padding: 2px 6px; border-radius: 4px;">#${sub.id_pedido}</span>
            </div>
            <div style="display:flex; gap:6px; align-items:center;">
              ${tallerActionsHTML}
              <button type="button" class="btn-attach-card" onclick="event.stopPropagation(); window.abrirAdjuntosPedido('${sub._docId}', this)" title="Adjuntar Archivo" style="width:26px; height:26px; display: flex; align-items: center; justify-content: center; padding:0; background: transparent; border: 1px solid var(--border); border-radius: 4px; cursor: pointer; color: var(--text-secondary);">
              ...
              <div class="product-dropdown" style="position: relative;">
                <button type="button" class="product-dropdown-toggle" data-card-dropdown-toggle="card-menu-${sub._docId}" aria-label="Acciones" style="width:20px; height:26px; padding:0; display:flex; align-items:center; justify-content:center; background: transparent; border: 1px solid var(--border); border-radius: 4px; cursor: pointer; color: var(--text-secondary);">⋮</button>
                <div class="product-dropdown-menu" id="card-menu-${sub._docId}">
                  <button type="button" class="product-dropdown-item" data-card-edit="${sub._docId}">Editar</button>
                  <button type="button" class="product-dropdown-item" data-card-remove="${sub._docId}">Eliminar</button>
                </div>
              </div>
            </div>
          </div>
          
          <div class="pedido-card-products" style="margin-left: 10px; border-left: 2px solid var(--accent, #e53935); padding-left: 8px; font-size:0.8rem; margin-bottom: 8px; color: var(--text-primary); line-height: 1.4;">
            ${preview}${extra}
          </div>
          
          ${adjuntosHTML}
          
          <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.75rem; margin-top:8px; color: var(--text-secondary);">
            ${estadoPagoHTML}
            <div class="pedido-card-date" title="${sub.fecha_creacion?.toDate?.()?.toLocaleString?.('es-MX') || ''}">${timeAgo(sub.fecha_creacion)}</div>
          </div>
        </div>
      `;
    }).join('');
    ...
    return `
      <div class="${pedido.isOptimistic ? 'pedido-card optimistic-saving' : 'pedido-card'}" data-doc-id="${pedido._docId}" style="${pedido.isOptimistic ? 'pointer-events: none; opacity: 0.6; animation: optimistic-pulse 1.5s infinite;' : ''}">
        <div style="display:flex; justify-content:space-between; align-items:center; padding-bottom: 8px; border-bottom: 2px solid var(--border); font-weight:700;">
          ...
          <!-- Botón de imprimir grupal -->
          <div style="display:flex; gap:6px; align-items:center;">
            <button type="button" class="btn-print-card" data-card-print="${pedido._docId}" title="Imprimir Recibo Grupal" style="width:28px; height:28px; display: flex; align-items: center; justify-content: center; padding:0; background: transparent; border: 1px solid var(--border); border-radius: 4px; cursor: pointer; color: var(--text-secondary);">
  ...
  // Comportamiento normal (pedido no unificado)
  const maxPreview = 3;
